...
export default function PastPerformance() {
  ...
  const addPerformance = () => {
    setPastPerformances(prev => [...prev, { client_name: '', contract_title: '', description: '', logo: '' }]);
  };
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        {pastPerformances.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mb-3">No past performance entries yet</p>
          ...
          <div className="space-y-4">
            {pastPerformances.map((perf, i) => (
              <div key={i} className="border border-gray-200 rounded-lg p-4 bg-gray-50/50 relative">
                <button onClick={() => removePerformance(i)} className="absolute top-3 right-3 p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors cursor-pointer">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 pr-6">
                    <div>
                      ...
                      <input type="text" value={perf.client_name || ''} onChange={(e) => updatePerformance(i, 'client_name', e.target.value)} placeholder="e.g., Department of Defense" className={INPUT_CLASS} />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-500 mb-1">Contract Title</label>
                      <input type="text" value={perf.contract_title || ''} onChange={(e) => updatePerformance(i, 'contract_title', e.target.value)} placeholder="e.g., IT Modernization Phase II" className={INPUT_CLASS} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-xs font-medium text-gray-500 mb-1">Description & Results</label>
                      <textarea value={perf.description || ''} onChange={(e) => updatePerformance(i, 'description', e.target.value)} placeholder="Describe the contract scope, your role, and key outcomes..." rows={2} className={`${INPUT_CLASS} resize-y`} />
                    </div>
                  </div>
                </div>
              </div>
